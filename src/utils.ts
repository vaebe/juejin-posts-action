...
const ASSETS_PATH = 'https://raw.https://github.com/vaebe/juejin-posts-action/main/assets/'
...
export function getTimeDiffString(timestamp: number | string): string {
  const now = Date.now()
  const time = typeof timestamp === 'string' ? Number(timestamp) * 1000 : timestamp * 1000
  const diff = now - time
...
export async function getJuejinList(user_id: string) {
  try {
    ...
    return data.map(({ article_info }: { article_info: JuejinArticleInfo }) => ({
